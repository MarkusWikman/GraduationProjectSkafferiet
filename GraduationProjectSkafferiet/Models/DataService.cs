...
namespace GraduationProjectSkafferiet.Models
{
    public class DataService
    {
        ...
        internal async Task<HomeVM> GetIngredientsAndInventoryAsync(HomeVM model)
        {
            ...
            string[] filteredIngredientsList = ingredientsList.Select(x => x).Except(q).ToArray();

            //Här är ingredients
            SelectListItem[] ingredients = new SelectListItem[filteredIngredientsList.Length];
            for (int i = 0; i < filteredIngredientsList.Length; i++)
            {
                ingredients[i] = new SelectListItem
                {
                    Value = filteredIngredientsList[i],
